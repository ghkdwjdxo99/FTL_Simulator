...
BOOL ftl_write(UINT32 startLBA, UINT32 sector_cnt)
{
	...
	for (UINT32 sector_offset = 0; sector_offset < sector_cnt; sector_offset++)
	{
		...
		// "한 페이지 단위" = sector_offset이 마지막 sector(sector_cnt-1)일 때 또는 128배수일 때  -> cursor 및 metadata 업데이트
		//if ((sector_offset == sector_cnt - 1) || (sector_offset % (MAX_SECTOR_PER_PAGE - 1) == 0))
		if ( (sector_offset == sector_cnt - 1) ||		// 한 페이지만 write
			( ((sector_offset + 1) % MAX_SECTOR_PER_PAGE == 0) && sector_offset >= MAX_SECTOR_PER_PAGE -1 ) )		// 2 페이지 이상 write
		{
			/* ========== (2) cursor 업데이트 ========== */
			...
			/* ========== (3) metadata 업데이트 ========== */	
			UINT32 firstLBA_in_page = 0;				// 현재 한 페이지에 들어가는 첫번째 LBA
			UINT32 sectors_in_page = 0;					// 현재 한 페이지에 들어가는 sector의 개수
...
#if 0
			if (sector_offset > MAX_SECTOR_PER_PAGE)	// sector_cnt가 128을 넘을 때
			{
				firstLBA_in_page = targetLBA;

				if(sector_offset == sector_cnt - 1)			// sector_offset이 마지막 sector일 때 계산식 사용
					sectors_in_page = sector_cnt - (page_cnt) * MAX_SECTOR_PER_PAGE;

				//if (sector_offset % (MAX_SECTOR_PER_PAGE - 1) == 0)		// sector_offset이 128배수 일 때는 128개
				else			// sector_offset이 128배수 일 때는 128개
					sectors_in_page = MAX_SECTOR_PER_PAGE;
			}

			else          // sector_cnt가 128 이하일 때
			{
				firstLBA_in_page = startLBA;
			...
			//if (sector_offset % (MAX_SECTOR_PER_PAGE - 1) == 0)		// sector_offset이 128배수 일 때는 128개
			//	sectors_in_page = MAX_SECTOR_PER_PAGE;
			//else				// sector_offset이 마지막 sector일 때는 계산식 이용
			//	sectors_in_page = sector_cnt - (page_cnt ) * MAX_SECTOR_PER_PAGE;		
#endif
	...
	for (UINT32 sector_offset = 0; sector_offset < sector_cnt; sector_offset++)
	{
		...
		while (1)
		{
			...
			cur_cursor = g_Cursor + block_offset;				// g_Cursor에서 현재 cursor를 구할 위치
			cur_page = get_block_write_page_idx(cur_cursor);	// 그 위치에 저장되어있는 cursor 값 = write할 수 있는 page
		...
		/* ========== (5) cursor 업데이트 ========== */
		// (*) sector_cnt를 128보다 크게 write할 때
		if (sector_cnt > MAX_SECTOR_PER_PAGE)		
		{
			if (sector_offset % MAX_SECTOR_PER_PAGE == 0)		// sector_offset이 128 배수일때 마다(한 페이지 채움) cursor 업데이트
			{
		...
		// (*) sector_cnt를 128이하로 write할 때
		else		
		{	
			if (sector_offset == sector_cnt - 1)		
			{
				set_cursor_next_page(cur_cursor);		// sector_offset이 sector_cnt-1 일때만 cursor 업데이트
